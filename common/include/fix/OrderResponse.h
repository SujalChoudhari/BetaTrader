...
namespace fix {
    ...
    public:
         ...
         */
        OrderResponse(const CompID &senderCompId,
            ...
            : mSenderCompId(senderCompId),
              ...
              mExchangeOrderId(exchangeOrderId),
              ...
              mSymbol(symbol),
              mSide(side),
              mOrderQuantity(orderQty),
              mCumulativeQuantity(cumQty),
              mLeavesQuantity(leavesQty),
              mLastPrice(lastPrice),
              mLastQuantity(lastQty),
        ...
        [[nodiscard]] SequenceNumber getMessageSequenceNumber() const { return mMessageSequenceNumber; }
        /** @brief Gets the exchange-assigned order ID (Tag 37). */
        [[nodiscard]] const ExchangeOrderID &getExchangeOrderId() const { return mExchangeOrderId; }
        ...
        [[nodiscard]] const std::string &getExecutionId() const { return mExecutionId; }
        ...
        [[nodiscard]] Quantity getOrderQuantity() const { return mOrderQuantity; }
        /** @brief Gets the cumulative filled quantity (Tag 14). */
        [[nodiscard]] Quantity getCumulativeQuantity() const { return mCumulativeQuantity; }
        /** @brief Gets the remaining quantity (Tag 151). */
        [[nodiscard]] Quantity getLeavesQuantity() const { return mLeavesQuantity; }
    ...
    private:
        ...
        // Identifiers
        ExchangeOrderID mExchangeOrderId; ///< Tag 37: Exchange-assigned order ID.
        ClientOrderID mClientOrderId; ///< Tag 11: Client-assigned order ID (echoed back).
        std::string mExecutionId; ///< Tag 17: Execution ID.

        // Status
        common::OrderStatus mStatus; ///< Tag 39: Order status.
        std::string mText; ///< Tag 58: Text message (e.g., reject reason, notes).
