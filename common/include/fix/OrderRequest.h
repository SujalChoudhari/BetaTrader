...
namespace fix {
    ...
    public:
         ...
         * @param stopPrice The stop price for stop or stop-limit orders (Tag
         * 99).
         * @param settlementType The settlement type (Tag 63).
         * @param settlementDate The settlement date (Tag 64).
         */
        OrderRequest(const CompID& senderCompId, const CompID& targetCompId,
                     const SequenceNumber msgSeqNum,
                     const ClientOrderID& clientOrderId, const Symbol& symbol,
                     const common::OrderSide side, const common::OrderType type,
                     const common::TimeInForce timeInForce,
                     const Quantity quantity, const Price price,
                     const Price stopPrice, const uint8_t settlementType,
                     const uint32_t settlementDate)
            : mSenderCompId(senderCompId), mTargetCompId(targetCompId),
              mMessageSequenceNumber(msgSeqNum), mClientOrderId(clientOrderId),
              mSymbol(symbol), mSide(side), mType(type),
              mTimeInForce(timeInForce), mQuantity(quantity), mPrice(price),
              mStopPrice(stopPrice), mSettlementType(settlementType),
        ...
        [[nodiscard]] SequenceNumber getMsgSeqNum() const
        {
        ...
        [[nodiscard]] common::TimeInForce getTimeInForce() const
        {
            return mTimeInForce;
        }
        ...
        [[nodiscard]] uint8_t getSettlementType() const
        {
            return mSettlementType;
        }
        /** @brief Gets the settlement date (Tag 64). */
        [[nodiscard]] uint32_t getSettlementDate() const
        {
    ...
    private:
        ...
        common::TimeInForce
                mTimeInForce; ///< Tag 59: Time in force for the order.
