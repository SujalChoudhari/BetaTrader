...
namespace common {
     ...
     *
     * This class is a data model that encapsulates all the properties of a
     * trade, including its ID, symbol, buy/sell order IDs, quantity, price, and
     * timestamp.
     */
    class Trade {
    public:
         ...
         */
        Trade(TradeID tradeId, Instrument symbol, OrderID buyOrderId,
              OrderID sellOrderId, const Quantity qty, const Price price,
              const Timestamp ts)
            : mTradeId(tradeId), mOrderSymbol(symbol), mBuyOrderId(buyOrderId),
              mSellOrderId(sellOrderId), mQuantity(qty), mPrice(price),
              mTimestamp(ts)
        {}
    ...
    public:
        ...
        [[nodiscard]] const Instrument& getOrderSymbol() const
        {
            return mOrderSymbol;
        }
        /** @brief Gets the buy order's unique identifier. */
        [[nodiscard]] const OrderID& getBuyOrderId() const
        {
            return mBuyOrderId;
        }
        /** @brief Gets the sell order's unique identifier. */
        [[nodiscard]] const OrderID& getSellOrderId() const
        {
            return mSellOrderId;
        }
    ...
    private:
        ...
        OrderID mSellOrderId; ///< The ID of the sell order involved in the
                              ///< trade.
