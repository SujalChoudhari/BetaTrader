...
namespace runbook {
    ...
    public:
        ...
        [[nodiscard]] std::string_view getWhat() const { return mWhatWentWrongString; }
        [[nodiscard]] std::string_view getHow() const { return mHowToFixString; }
    ...
    static std::string FormatRunbookLog(const ErrorDefinition &errorDef, const std::string_view &fmt_str,
                                        Args &&... args) {
        std::string user_msg = fmt::format(fmt::runtime(fmt_str), std::forward<Args>(args)...);
