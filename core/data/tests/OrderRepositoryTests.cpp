...
TEST_F(OrderRepositoryTests, SaveAndLoadOrder)
{
    ...
    orderRepository->loadOrdersForInstrument(
            ...
            [&promise](std::vector<common::Order> orders) {
    ...
    EXPECT_EQ(loadedOrders[0].getOriginalQuantity(),
              order.getOriginalQuantity());
    EXPECT_EQ(loadedOrders[0].getRemainingQuantity(),
...
TEST_F(OrderRepositoryTests, UpdateOrder)
{
    ...
    EXPECT_EQ(loadedOrders[0].getStatus(),
