...
public:
    // This constructor will now be called with a valid MockDatabaseWorker pointer
    using data::TradeRepository::TradeRepository;

    // We no longer need to override initDatabase because the mock worker's enqueue does nothing
...
protected:
    ...
    void SetUp() override {
        ...
        mockTradeRepo = std::make_unique<MockTradeRepository>(mockDbWorker.get());
        
        // Now, create the object under test with valid mock pointers
        riskManager = std::make_unique<trading_core::RiskManager>(mockTradeRepo.get());
