...
TEST(ExecutionPublisherTests, PublishExecution) {
    ...
    std::string expected_output = "[ExecutionPublisher] EXECUTION | Action=PARTIAL_FILL | OrderID=123 | Symbol=EURUSD | Qty=50 | Price=1.250000 | Client=client1\n";
    ASSERT_EQ(buffer.str(), expected_output);
}

TEST(ExecutionPublisherTests, PublishTrade) {
    common::Trade trade(789, common::Instrument::GBPUSD, 456, 123, 50, 1.2505, std::chrono::system_clock::now());
    ...
    expected_output_stream << "[ExecutionPublisher] TRADE | TradeID=789 | BuyOrder=456 | SellOrder=123 | Qty=50 | Price=1.250500 | Timestamp=" << timestamp_us << "us\n";
...
TEST(ExecutionPublisherTests, PublishRejection) {
    ...
    {
        CoutRedirector redirect(buffer);
        trading_core::ExecutionPublisher::publishRejection(orderId, clientId, reason);
    }

    std::string expected_output = "[ExecutionPublisher] REJECT | OrderID=999 | Client=client-abc | Reason=Insufficient funds\n";
    ASSERT_EQ(buffer.str(), expected_output);
}
