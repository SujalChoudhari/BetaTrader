...
namespace trading_core {
            ...
            if (!bestPriceLevel.empty()) {
                ...
                if (bestOrder->getClientId() == order.getClientId()) {
                    if (order.getOrderType() == common::OrderType::Market) {
                        LOG_ERROR("ETRADE12", "Pre-check failed for order ID {}: Self-match detected with market order.", order.getId());
                        return true; // Self-match detected
                    }
                    else if (order.getSide() == common::OrderSide::Buy && order.getPrice() >= bestOrder->getPrice()) {
                        LOG_ERROR("ETRADE12", "Pre-check failed for order ID {}: Self-match detected with limit order.", order.getId());
                        return true; // Self-match detected
                    }
                    else if (order.getSide() == common::OrderSide::Sell && order.getPrice() <= bestOrder->getPrice()) {
                        LOG_ERROR("ETRADE12", "Pre-check failed for order ID {}: Self-match detected with limit order.", order.getId());
                        return true; // Self-match detected
                    }
                }
    ...
    bool RiskManager::preCheck(const common::Order &order, OrderBook& orderBook) {
        // Basic sanity checks
        if (order.getOriginalQuantity() <= 0 || (order.getOrderType() == common::OrderType::Limit && order.getPrice() <= 0)) {
            LOG_ERROR("ETRADE10", "Pre-check failed for order ID {}: Invalid quantity or price.", order.getId());
            return false;
        }
        ...
        if (order.getOrderType() == common::OrderType::Limit) {
            ...
            if (order.getSide() == common::OrderSide::Buy && !orderBook.getAskMap()->empty()) {
                topOfBookPrice = orderBook.getAskMap()->begin()->first;
            } else if (order.getSide() == common::OrderSide::Sell && !orderBook.getBidMap()->empty()) {
                topOfBookPrice = orderBook.getBidMap()->begin()->first;
            }

            if (topOfBookPrice > 0) {
                double deviation = std::abs(order.getPrice() - topOfBookPrice) / topOfBookPrice;
                if (deviation > MAX_PRICE_DEVIATION) {
                    LOG_ERROR("ETRADE11", "Pre-check failed for order ID {}: Price deviates too much from top of book.", order.getId());
                    return false;
                }
            }
        }
