...
namespace trading_core {
    Partition::Partition(common::Instrument symbol, data::DatabaseWorker *databaseWorker,
    ...
    void Partition::init() {
        ...
        mOrderRepository->loadOrdersForInstrument(mSymbol, [this, loadPromise](std::vector<common::Order> orders) {
            for (auto &order: orders) {
                auto orderUniquePtr = std::make_unique<common::Order>(order);
                mOrderBook->insertOrder(orderUniquePtr.get());
                mOrderManager->addOrder(std::move(orderUniquePtr));
            }
            LOG_INFO("Loaded {} orders for symbol {}", orders.size(), common::to_string(mSymbol));
            loadPromise->set_value(); // Signal that loading is complete
        });

        LOG_INFO("Partition for symbol {} constructed and loading started.", common::to_string(mSymbol));
    }
    ...
    void Partition::start() {
        ...
        mWorker = std::make_unique<WorkerThread>(mCommandQueue, *mOrderManager, *mOrderBook, *mMatcher, *mRiskManager,
                                                 *mOrderRepository, mTradeIDGenerator, mDatabaseWorker);
        mWorker->start();
