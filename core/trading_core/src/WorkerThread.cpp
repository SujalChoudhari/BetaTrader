...
namespace trading_core {
    WorkerThread::WorkerThread(
            ...
            OrderManager& orderManager, OrderBook& orderBook, Matcher& matcher,
            RiskManager& riskManager, data::OrderRepository& orderRepository,
            TradeIDGenerator* tradeIDGenerator,
            data::DatabaseWorker* databaseWorker)
        : mCommandQueue(commandQueue), mOrderManager(orderManager),
          mOrderBook(orderBook), mMatcher(matcher), mRiskManager(riskManager),
          mOrderRepository(orderRepository),
          mTradeIDGenerator(tradeIDGenerator), mDatabaseWorker(databaseWorker),
          mCommandBatch{}
    {}

    WorkerThread::~WorkerThread()
    {
        if (mThread.joinable()) { stop(); }
    }
    ...
    void WorkerThread::start()
    {
        ...
        mThread = std::jthread(
                [this](std::stop_token st) { this->runLoop(st); });
    }
    ...
    void WorkerThread::runLoop(std::stop_token stopToken)
    {
        ...
        while (!stopToken.stop_requested()) { processNextCommand(); }
    ...
    void WorkerThread::processNextCommand()
    {
        ...
        if (dequeued > 0) { processBatch(mCommandBatch, dequeued); }
        else {
            std::this_thread::yield();
        }
    }

    void WorkerThread::processBatch(std::unique_ptr<Command>* commands,
                                    size_t count)
    {
        for (size_t i = 0; i < count; ++i) {
            ...
            case CommandType::NewOrder:
                processNewOrder(*static_cast<NewOrder*>(cmd.get()));
                break;
            case CommandType::CancelOrder:
                processCancelOrder(*static_cast<CancelOrder*>(cmd.get()));
                break;
            case CommandType::ModifyOrder:
                processModifyOrder(*static_cast<ModifyOrder*>(cmd.get()));
                break;
            default:
                LOG_ERROR(errors::ETRADE1, "Found type: {}",
                          trading_core::to_string(cmd->getType()));
                break;
            }
        }
    }
    ...
    void WorkerThread::processCancelOrder(const CancelOrder& cmd) const
    {
        ...
        if (!orderOpt) {
            ExecutionPublisher::publishRejection(
                    cmd.getOrderId(), cmd.getClientId(), "Order not found");
            return;
        }
