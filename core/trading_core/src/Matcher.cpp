...
namespace trading_core {
    Matcher::Matcher(TradeIDGenerator* tradeIdGenerator)
        : mTradeIdGenerator(tradeIdGenerator)
    {}

    std::vector<common::Trade> Matcher::match(common::Order* incomingOrder,
                                              OrderBook& orderBook)
    {
                             ...
                             std::vector<common::Trade>& trades)
    {
        ...
        while (it != restingMap->end()
            ...
            while (!restingLevel.empty()
                   && incomingOrder->getRemainingQuantity() > 0) {
                auto restingOrder = restingLevel.front();

                common::Quantity tradeQuantity
                        = std::min(incomingOrder->getRemainingQuantity(),
                ...
                trades.emplace_back(newId, incomingOrder->getSymbol(), buyId,
                                    sellId, tradeQuantity, tradePrice,
                                    std::chrono::system_clock::now());
                LOG_INFO("New trade created with id {}", newId);

                incomingOrder->setRemainingQuantity(
                        incomingOrder->getRemainingQuantity() - tradeQuantity);
                restingOrder->setRemainingQuantity(
                        restingOrder->getRemainingQuantity() - tradeQuantity);
            ...
            if (restingLevel.empty()) { it = restingMap->erase(it); }
            else {
                ++it;
            }
        }
        ...
        if (incomingOrder->getRemainingQuantity() == 0) {
            ...
            // If a market order is not fully filled, the remainder is
            // cancelled. If it was never filled at all, its original status
            // (e.g., New) should be updated to Cancel.
            if (incomingOrder->getOriginalQuantity()
                == incomingOrder->getRemainingQuantity()) {
                incomingOrder->setStatus(common::OrderStatus::Cancelled);
            }
            // If it was partially filled, its status is already
            // PartiallyFilled, which is a valid final state for an IOC
