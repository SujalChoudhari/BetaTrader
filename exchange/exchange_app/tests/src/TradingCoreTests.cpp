...
TEST_F(TradingCoreTest, DependencyInjectionConstructor) {
    auto mockDb = std::make_unique<MockDatabaseWorker>();
    auto mockAuth = std::make_unique<MockAuthRepository>(mockDb.get());
    auto mockTradeRepo = std::make_unique<MockTradeIDRepository>(mockDb.get());
    auto mockTradeGen = std::make_unique<MockTradeIDGenerator>(mockTradeRepo.get());
    auto mockOrderGen = std::make_unique<MockOrderIDGenerator>(mockDb.get());

    TradingCore core(std::move(mockDb), std::move(mockAuth),
                     std::move(mockTradeRepo), std::move(mockTradeGen),
                     std::move(mockOrderGen), true);
