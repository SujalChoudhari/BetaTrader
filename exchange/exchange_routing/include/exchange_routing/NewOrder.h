...
namespace trading_core {
     ...
     * required to create a new order, including the client ID, timestamp, and a
     * unique pointer to the Order object.
     */
    ...
    public:
         ...
         * @param order A unique pointer to the common::Order object being
         * submitted.
         */
        explicit NewOrder(const common::ClientID& clientId,
                          const common::Timestamp timestamp,
                          std::unique_ptr<common::Order> order)
            : Command(CommandType::NewOrder, clientId, timestamp),
         ...
         */
        [[nodiscard]] std::unique_ptr<common::Order> releaseOrder()
        {
            return std::move(mOrder);
        }

    private:
        std::unique_ptr<common::Order>
                mOrder; ///< A unique pointer to the new order.
    };
} // namespace trading_core
